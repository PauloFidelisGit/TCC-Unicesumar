...
import { mapQueryDTOToView } from "./search-municipio.dto";
import { QUERY_SEARCH_MUNICIPIO } from "./search-municipio.queries";
import { formSchema, FormSchema } from "./search-municipio.schema";

export default function SearchMunicipioPage() {
  const form = useForm({
  ...
  const municipio = data?.searchMunicipio
    ? mapQueryDTOToView(data.searchMunicipio)
    : [];
  ...
  return (
    <DashboardPageContainer className="space-y-4">
      <DashBoardPageTitle>Buscar Município</DashBoardPageTitle>
            ...
            <Card className="mt-4">
              <CardContent className="pt-4">
                <Table>
                  ...
                  <TableBody>
                    {municipio?.map(({ uuid, nome }, index) => (
                      <TableRow key={index}>
                        <TableCell>
                          <NavLink
                            to={navAdvogado.dashboard.advogado.toMunicipio({
                              municipio_uuid: uuid,
                            })}
                          >
                            {nome}
