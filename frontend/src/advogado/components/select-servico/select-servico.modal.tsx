...
import type { SearchServicoArgs } from "backend/types";
import { useForm } from "react-hook-form";
import { toast } from "react-toastify";
import { QUERY_SEARCH_SERVICO } from "./select-servico.queries";
import { FormSchema, formSchema } from "./select-servico.schema";
import { mapQueryPickSearchServicoDTOToView } from "./selected-servico.dto";

interface Props {
  control: UseSelectModalControl;
}
export function SelectServicoModal({
  ...
  const [search, { loading, data, called }] =
    useLazyQuery(QUERY_SEARCH_SERVICO);

  const servico = mapQueryPickSearchServicoDTOToView(data?.searchServico);
  ...
  async function onSubmit(data: FormSchema) {
    const { loading } = await search({
      variables: {
        value: data.value,
        limit: 10,
        where: data.where as SearchServicoArgs["where"],
      },
    });
